...
const displayRunning = (myFavCar) => {
  const isRunning = document.getElementById("is-running");
  const result = myFavCar.carIsRunning();
  result === true ? isRunning.innerHTML = "Car is running." : isRunning.innerHTML = "Car is not running."
}

const addEvents = (myFavCar) => {
  const accelerateButton = document.getElementById("accelerate");
  accelerateButton.addEventListener("click", () => { accelerate(myFavCar), checkRunning(myFavCar) });
  const brakeButton = document.getElementById("brake");
  brakeButton.addEventListener("click", () => { brake(myFavCar), checkRunning(myFavCar) });
  const stopButton = document.getElementById("stop");
  stopButton.addEventListener("click", () => { stop(myFavCar), checkRunning(myFavCar) });
};
...
const checkRunning = (myFavCar) => {
  myFavCar.carIsRunning();
  displayRunning(myFavCar);
}
